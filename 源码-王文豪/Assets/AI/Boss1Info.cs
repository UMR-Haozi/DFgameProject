...
public class Boss1Info : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (isTrigger)
        {
            ...
            {
                gameObject.GetComponent<Renderer>().material.mainTexture = (Texture)Resources.Load("Img/Boss1");
            }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (life == 0)
        {
            GameObject.Find("Player").GetComponent<PlayerInfo>().addScore(2000);
            Destroy(gameObject);
            return;
        }
    }
